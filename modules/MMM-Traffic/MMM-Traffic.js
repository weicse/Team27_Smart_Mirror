...
/*
Modified by Team 27
-Added notification recieving function to work with Remote Control module
-Switch between walking/driving remotely: [IMPLEMENTED]
-Update origin/destination remotely: [In Progress]
*/
Module.register('MMM-Traffic', {
	...
	getDom: function() {
		...
		if (this.config.changeColor) {
			if (this.trafficComparison >= 1 + (this.config.limitRed / 100)) {
				// Cause the displayed text to turn red if traffic accounts for limitRed % of the commute time
				commuteInfo.className += ' red';
			}
			else if (this.trafficComparison >= 1 + (this.config.limitYellow / 100)) {
				// Cause the displayed text to turn red if traffic accounts for limitYellow % of the commute time
	...
	getTodaysArrivalTime: function() {
		...
		if (!final_arrival_time){
			// if no weekday arrival_times defined in config, set to default
